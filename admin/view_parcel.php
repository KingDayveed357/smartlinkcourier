...
$qry = $conn->query("SELECT * FROM parcels where id = " . (int)$_GET['id'])->fetch_array();
foreach ($qry as $k => $v) {
    $$k = $v;
}

// Fetch branch details
if (isset($to_branch_id) && isset($from_branch_id) && ($to_branch_id > 0 || $from_branch_id > 0)) {
...
$shipping_history = $conn->query("SELECT * FROM shipping_history WHERE parcel_id = ".$_GET['id']." ORDER BY created_at DESC");
?>
...
<div class="container-fluid">
    <div class="col-lg-12">
            ...
            <div class="col-md-6">
                <div class="callout callout-info">
                    ...
                    <dt>Branch Accepted the Parcel:</dt>
                        <dd><?php echo ucwords($branch[$from_branch_id] ?? 'N/A') ?></dd>
                        <?php if ($type == 2): ?>
                            <dt>Nearest Branch to Recipient for Pickup:</dt>
                            <dd><?php echo ucwords($branch[$to_branch_id] ?? 'N/A') ?></dd>
                        <?php endif; ?>
                        <dt>Status:</dt>
                        <dd>
                            <?php 
                            switch ($status) {
                                case '1':
                                    echo "<span class='badge badge-pill badge-info'> Shipment Plan Started</span>";
                                    break;
                                case '2':
                                    echo "<span class='badge badge-pill badge-info'>Shipment in transit</span>";
                                    break;
                                case '3':
                                ...
                                case '8':
                                    ...
                                    break;
                                case '9':
                                    echo "<span class='badge badge-pill badge-danger'> On-Hold</span>";
                                    break;
                                default:
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="callout callout-info">
                    <b class="border-bottom border-primary">Shipping History</b>
                    <?php if($shipping_history->num_rows > 0): ?>
                <?php while($history = $shipping_history->fetch_assoc()): ?>
                        <dl>
                            <dt>Date:</dt>
                            <dd><?php echo date('Y-m-d H:i:s', strtotime($history['timestamp'])) ?></dd>
                            <dt>Status:</dt>
                                 <?php 
                            ...
                            <dt>Location:</dt>
                            <dd><?php echo ucwords($history['location']) ?></dd>
                            <dt>Comments:</dt>
                            <dd><?php echo ucwords($history['comments']) ?></dd>
                        </dl>
                    <?php endwhile; ?>
                    <?php else: ?>
                        <p>No record found</p>
                    <?php endif; ?>
    <span class="btn badge badge-primary bg-gradient-primary" id='update_history'><i class="fa fa-edit"></i> Update Shipping history</span>
                </div>
            </div>
        </div>
    </div>
...
<script>
    $('#update_status').click(function(){
       ...
       $('#update_history').click(function(){
        uni_modal("Update Shipping History of: <?php echo $reference_number ?>","manage_parcel_history.php?id=<?php echo $id ?>&cs=<?php echo $status ?>","")
    })
</script>
